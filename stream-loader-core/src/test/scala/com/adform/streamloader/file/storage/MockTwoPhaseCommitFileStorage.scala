...
class MockTwoPhaseCommitFileStorage extends TwoPhaseCommitFileStorage[Unit] {
  ...
  override def stageFile(file: RecordRangeFile[Unit]): FileStaging = {
    stagesInvoked += 1
    if (stagesInvoked == failOnStage) {
      throw new UnsupportedOperationException("File staging failed")
    }
    val path = file.file.getAbsolutePath
  ...
  override def storeFile(fileStaging: FileStaging): Unit = {
    storesInvoked += 1
  ...
  override def isFileStored(fileStaging: FileStaging): Boolean = storedFiles.contains(fileStaging.destinationPath)
}
