...
class InDataOffsetFileStorageTest extends AnyFunSpec with Matchers {
  ...
  class MockInDataOffsetFileStorage extends InDataOffsetFileStorage[Unit] {
    val storedFiles: ListBuffer[File] = ListBuffer.empty[File]
    override def storeFile(file: RecordRangeFile[Unit]): Unit = storedFiles.addOne(file.file)
